...
public class DocumentEntity extends BaseEntity {
    ...
    @NotNull
    @Column(name = "doc_id", length = 255, unique = true, nullable = false)
    String docid;

    @Enumerated(EnumType.STRING)
    @Column(name = "doc_source", length = 255, nullable = false)
    DocSource docSource;

    @Enumerated(EnumType.STRING)
    @Column(name = "doc_format", length = 255, nullable = false)
    DocFormat docFormat;

    @Column(name = "doc_data", length = 2048000)
    String docData;
