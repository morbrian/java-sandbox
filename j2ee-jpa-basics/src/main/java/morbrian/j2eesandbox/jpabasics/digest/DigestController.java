...
public class DigestController {
    ...
    public void processData(String docData, DocSource docSource, Calendar docRetrievedDate) {
        ...
        RawDocumentEntity rawEntity = storeRawDocument(docData, docSource, DocFormat.STRING, docRetrievedDate);
    ...
    void processTrimmedData(String trimmedData, RawDocumentEntity rawEntity, DocSource docSource) {
        ObjectMapper mapper = new ObjectMapper();
            try {
                ...
                DerivedDocumentEntity docEntity = documentRepo.findDerivedDocumentBySourceAndChecksum(docSource, checksum);
                if (docEntity == null) {
                    storeAsJsonDocument(derivedData, checksum, rawEntity, docSource, DocType.UNITSTATUS, DocFormat.JSON);
                } else {
                    log.info("Record already exists for {docSource=" + docSource +", checksum=" + checksum + " }");
                }
    ...
    void storeAsJsonDocument(String derivedData, String checksum, RawDocumentEntity rawEntity, DocSource docSource, DocType docType, DocFormat docFormat) {
