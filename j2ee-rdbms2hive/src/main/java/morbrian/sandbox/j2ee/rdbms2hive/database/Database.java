...
public class Database {
    ...
    private static Set<String> SUPPORTED_SCHEMAS = Collections.unmodifiableSet(new HashSet(Arrays.asList("public")));
    private static Set<String> SUPPORTED_TABLES = Collections.unmodifiableSet(new HashSet(Arrays.asList("tablenames")));

    @Inject
    private Logger log;

    @Resource(mappedName="java:/DocumentDS")
    ...
    public NamesAndRecords fetchData(String query) {
        ...
        try {
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                log.info("Col " + metaData.getColumnName(i) + " MetaData: " + metaData.getColumnTypeName(i));
            }
            ...
            while(resultSet.next()) {
                ...
                for (int i = 1; i <= columnCount; i++) {
                    ...
                    log.info("Col " + metaData.getColumnName(i) + " Java Type: " + (obj == null ? "null" : obj.getClass().getName()));
                    record.add(obj);
                }
                records.add(record);
            }
            ...
            if (resultSet != null) {
                try { resultSet.close(); } catch(SQLException exc2) {}
            }
            if (stmt != null) {
                try { stmt.close(); } catch(SQLException exc2) {}
            }
            if (conn != null) {
                try { conn.close(); } catch(SQLException exc2) {}
            }
        }
